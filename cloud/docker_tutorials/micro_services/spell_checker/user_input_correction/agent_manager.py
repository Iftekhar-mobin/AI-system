...
class AgentManager:
    def __init__(self):
        print("[Info] Spelling Checker: init()")
        self.agents = []

    def get_agent(self, agent_id, data_dir):
        self._set_low_priority_to_agent()
        current_agent = self._get_current_agent(agent_id)
        if current_agent is None:
            current_agent = self._add_new(agent_id, data_dir)
    ...
    def _add_new(self, agent_id, data_dir):
        agent = Agent(agent_id, data_dir)
