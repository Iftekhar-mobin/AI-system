...
class AgentManager:
    ...
    def get_agent(self, agent_id, model_path):
        self._set_low_priority_to_agent()
        current_agent = self._get_current_agent(agent_id)
        if current_agent is None:
            current_agent = self._add_new(agent_id, model_path)
    ...
    def _add_new(self, agent_id, model_path):
        agent = Agent(agent_id, model_path)
