...
class AgentManager:
    def __init__(self):
        logger.info("Sentiment Analyzer: init()")
        self.agents = []

    def get_agent(self, agent_id, model_path, data_dir, model_dir):
        self._set_low_priority_to_agent()
        current_agent = self._get_current_agent(agent_id)
        if current_agent is None:
            current_agent = self._add_new(agent_id, model_path, data_dir, model_dir)
    ...
    def _add_new(self, agent_id, model_path, data_dir, model_dir):
        agent = Agent(agent_id, model_path, data_dir, model_dir)
